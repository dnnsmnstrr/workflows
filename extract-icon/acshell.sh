...
###################################
	...
	if [[ "$qr" == "online" ]]; then
		osascript -e 'tell application "Alfred 2" to run trigger "online" in workflow "com.mcskrzypczak.extracticon" with argument "'"$argument"'"'
	else
		...
		osascript -e 'tell application "Alfred 2" to run trigger "notify" in workflow "com.mcskrzypczak.extracticon" with argument "'"$HOME/Desktop/$appname.png"'"'
	fi
